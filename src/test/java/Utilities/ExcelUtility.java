...
public class ExcelUtility {
    ...
    public static void writeExcel(String path, Scenario senaryo, String browserName) {
        ...
        if (!file.exists()) {
            XSSFWorkbook workbook = new XSSFWorkbook(); // yeni excel oluşturuluyor
            ...
            yeniHucre.setCellValue(senaryo.getStatus().toString());// fail veya pass diye verecek

            yeniHucre = yenisatir.createCell(2);
            yeniHucre.setCellValue(browserName);


            try { // dosya kaydetme işlemi
